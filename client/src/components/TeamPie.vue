...
<div>
  ...
  <v-app>
    ...
    >
      <template v-slot:top>
        <v-app-bar fluid color="cyan">
          <v-toolbar-title>Washington Wizards</v-toolbar-title>
          <v-divider
            class="mx-10"
            inset
            vertical
    ...
    >
      ...
      <v-card-text>
        <div class="display-1 font-weight-thin">PIE     Washington Wizards</div>
      </v-card-text>
...
export default {
  ...
  async mounted () {
    // this.players = (await PlayersServices.index()).data
    await (Api().get(`/api/players/${5}`).then((response) => {
      this.players = response.data
    }).catch((err) => {
      console.log('err in axios is ' + err)
    }))
