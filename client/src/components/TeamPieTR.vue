...
<div>
  ...
  <v-app>
    ...
    >
      <template v-slot:top>
        <v-app-bar fluid color="cyan">
          <v-toolbar-title>Toronto Raptors</v-toolbar-title>
          <v-divider
            class="mx-10"
            inset
            vertical
    ...
    >
      ...
      <v-card-text>
        <div class="display-1 font-weight-thin">PIE  Toronto Raptors</div>
      </v-card-text>
...
export default {
  ...
  data () {
    return {
      dialog: false,
      pie: [12.3, 11.2, 9.2, 12.3, 9.9, 13.2],
  ...
  async mounted () {
    // this.players = (await PlayersServices.index()).data
    await (Api().get(`/api/players/${10}`).then((response) => {
