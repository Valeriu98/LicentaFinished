...
<div>
  ...
  <v-app>
    ...
    >
      <template v-slot:top>
        <v-app-bar fluid color="cyan">
          <v-toolbar-title>Los Angeles Lakers </v-toolbar-title>
          <v-divider
            class="mx-10"
            inset
            vertical
    ...
    >
      ...
      <v-card-text>
        <div class="display-1 font-weight-thin">PIE    Los Angeles Lakers</div>
      </v-card-text>
...
export default {
  ...
  async mounted () {
    // this.players = (await PlayersServices.index()).data
    await (Api().get(`/api/players/${4}`).then((response) => {
